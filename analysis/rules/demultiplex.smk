...
rule demultiplex:
    ...
    run:
        ...
        if is_multiplexed:
            ...
            # deepbinner classify
            shell("deepbinner classify --native {input.fast5s} > {classification_path} 2> {log}")
            # deepbinner bin
            shell("deepbinner bin --classes {classifications_path} --reads {input.fastq} --out_dir {params.out_dir} 2> {log}")
