#!/bin/bash
# adapted from https://github.com/hasindu2008/slow5tools/blob/ad57e036c1e5ae557a10882da58b5bb73e373d45/scripts/mixed-multi-fast5-to-blow5.sh
...
read_id_based_classify_func() {
    ...
    test -z $RUN_ID && {
        echo "Could not deduce run_id in $file"
        exit 1
    }

    mkdir -p "$TMP_FAST5/$RUN_ID"
    mv -i "$file" "$TMP_FAST5/$RUN_ID" || {
        echo "moving $file to $TMP_FAST5/$RUN_ID failed"
        exit 1
    }
    # done
...
NUM_READS=$(find "$FAST5_DIR" -name '*.fast5' | parallel -I% --max-args 1 strings % | grep -c "read_.*-.*-.*" | awk 'BEGIN {count=0;} {count=count+$0} END {print count;}')
